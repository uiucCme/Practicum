...
def full_order_book_output(time, bid, ask):
    array_value = []
...
def ten_orderbook_output(time, bid, ask):
    array_value = [0]*42
    array_value[0] = time
    bid_position = 1
    ask_position = 22
...
data_input_dir = "/Users/luoy2/OneDrive/Documents/UIUC/CME Fall 2016/data/order_book/"
data_output_dir = "/Users/luoy2/OneDrive/Documents/UIUC/CME Fall 2016/data/HDF5/hdf5/"
...
stock_input = stock_input.loc[:, (stock_input != 0).any(axis=0)].reset_index(drop=True)
...
order_book = []
f = h5py.File("data/order_book/order_book_compressed.hdf5", 'w', compression="gzip", compression_opts=9)
...
for iteration in trange(len(stock_input)):
    ...
    temp_list = ten_orderbook_output(information['time'], bid, ask)
