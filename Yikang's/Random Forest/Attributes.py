...
def data_cleaning(df_input):
    ...
    end_time = '15:00:00'
    ftr = [3600, 60, 1]
    start_timestamp = sum([a * b for a, b in zip(ftr, [int(i)
                          for i in start_time.split(":")])]) * 1e+9
    end_timestamp = sum([a * b for a, b in zip(ftr, [int(i)
                        for i in end_time.split(":")])]) * 1e+9

    execution_message = df_input[df_input[1].isin(
        [ord('E'), ord('C')])].reset_index(drop=True)
    execution_message = execution_message[
        start_timestamp <= execution_message[0]]
    execution_message = execution_message[
        execution_message[0] <= end_timestamp].reset_index(drop=True)
...
# no need for time, message type and bid ask divide
execution_orderbook_df.drop([0, 1, 22], 1, inplace=True)
# only need price and quantity
execution_record_df.drop([0, 1], 1, inplace=True)
...
execution_record_pctchange_df['exe.p.d'] = execution_record_pctchange_df[
    'exe.p.d'].apply(get_direction)
...
for i in range(1, 11):
    imbalance_df['imbalance.' +
        str(i)] = get_imbalance_rate(execution_orderbook_df, i)
